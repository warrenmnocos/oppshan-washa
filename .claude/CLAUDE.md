...
- **Render diagrams to view.** After any SVG edit in `docs/diagrams/`, render to PNG via Chrome headless and `Read` the PNG — Claude reads PNGs as multimodal input. Coordinate math doesn't catch marker scaling, label-vs-line overlap, or marker skew on diagonal lines.
  ...
  A dense full-page diagram renders to a thumbnail too small to judge text fit or overflow; crop the PNG to the changed regions and upscale before reading: `convert /tmp/diagram.png -crop WxH+X+Y +repage -scale 230% /tmp/crop.png`. Raster-cropping an existing PNG with `convert` is fine (the warning above is only about SVG→PNG).

- **Guarded bulk renames use `perl -i.bak -pe`, not the Edit tool.** Edit is literal-only (no regex/lookbehind), so a sweep that must protect a substring (`com/oppshan/washa` while rewriting `/washa/`) or avoid double-prefixing (`washa-lambda-exec`) needs `perl -i.bak -pe 's/(?<!oppshan-)…/…/g'`. Keep the `.bak` (or `cp` to `/tmp`) for gitignored files: they have no `git checkout` to recover a bad regex. Grep-verify, then remove the `.bak`.

- **Recursive repo greps must `--exclude-dir=node_modules`** (and skip `/target/`): `src/main/angular/node_modules/` is large enough that an unscoped `grep -r` floods the output (~2MB this session).

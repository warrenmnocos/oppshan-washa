...
## Update — 2026-06-30 (test-infra session)

The parallel test-infra session, separate from the deps/PGO work above (see [[feedback_parallel_session_commits]]).

- **Pushed non-model line coverage toward 100% (partial, deliberate stop).** New tests: `DomainEnumTest` (every enum `fromValue` + unknown-throw, `BracketOp.holds`, `BracketType.contribution`), `DebtSimulatorTest` term-derivation, more `SalaryEngineTest` branches, `SalaryPresetMapperTest` (variable/bracket mapping), `YearMonthStringConverterTest`, `AuthSupportTest`, an `AllowedIdentitiesParser` malformed-JSON case — non-model missed lines **152 → ~78**, overall **~73%** (above the 70% CI gate). The rest is written up per-class with approaches + a jacoco-exclude proposal in **(gitignored) `docs/superpowers/specs/2026-06-30-non-model-coverage-gaps.md`**: the big tractable one is `BudgetService` (20 lines — a rich `compute` IT with a RELATIVE + each-period-unit TIME goal, a prepay-currency debt, and workingRates); the ~42 hard lines are OIDC sign-in/out, `StatefulWriteRepository` DB-error arms, and the Flyway-masked `@Startup` seeds. Warren chose "cover the tractable bulk, flag the rest."
- **Testcontainers reuse is OFF — the A.10 sync flagged just above is done (`e2d4e28`).** `testcontainers.reuse.enable=false` on the machine **and** `%test...devservices.reuse=false` in the build (`7131c98`), so Ryuk reaps the Dev Services Postgres per run (pass or fail) and nothing accumulates across sequential runs. The live hazard is the *within-run* DB shared across test classes (unique-per-run keys) and *overlapping* runs sharing one container via Dev Services `shared`. The `year_month` flake that bit local runs was **measurement chaos** (overlapping detached `./mvnw` launches + `kill -9` taking Ryuk down before it reaped), **not reuse** — CI (fresh container) never hits it; `BudgetServiceTest` uses a random per-run base year (`2701ffa`).
- **`scripts/test.sh` added** — a local-dev wrapper (`./mvnw -B "$@"` + an `EXIT` trap that force-reaps `postgres:18`) so a run killed before Ryuk fires can't leave orphans. Deliberately not in CI (the runner is discarded); this is the "local-dev wrapper worth it" carve-out in `.claude/CLAUDE.md`.
- **All Mockito tests now on `@Mock` + BDDMockito** (`29a4af8`, `81d0281`): no inline `mock()`/`when()`/`verify()` left in `src/test/`. Non-obvious gotcha (now in **A.10**): `@ExtendWith(MockitoExtension.class)` **silently no-ops under `@QuarkusTest`** (different test instance → null `@Mock` fields), so a non-CDI fixture mock there uses `@Mock` + `MockitoAnnotations.openMocks(this)` in `@BeforeEach`; plain unit tests keep MockitoExtension.
- **CI/CD split** (earlier this session): `cd.yml` runs the PGO native build only on push to `main` (paths filter: `src/main/**`, `pom.xml`, `.mvn/**`, `mvnw`, `scripts/graalvm-pgo/**`) and dropped `-DskipTests` so the suite gates the native build; `ci.yml` is PR-only.

...
The `oppshan-washa` Lambda + execution role + log group, the `AWS_IAM` Function URL + the two CloudFront invoke permissions (`InvokeFunctionUrl` + `InvokeFunction` — OAC→Lambda needs both), the `oppshan-washa-oac` Origin Access Control, the CloudFront distribution (three behaviors + the forwarded-host headers), the **us-east-1** ACM certificate (**ECDSA P-384**) + its DNS validation, the `washa.oppshan.com` Route 53 aliases, the ten `/oppshan/washa/*` SSM parameter slots, and the `oppshan-washa-github-deploy` role.

## What stays manual (Terraform doesn't touch)
- The **Neon** database (external managed service) — provision it via the console ([manual guide Phase 0](aws-deployment-manual.md#phase-0-provision-the-neon-database-console)) or `neonctl` ([CLI guide Phase 0](aws-deployment-cli.md#phase-0-provision-the-neon-database-neonctl)); set up the two roles per that Phase 0 (`washa_user` on the pooled endpoint, `washa_admin` on the direct) and have both connection sets ready.

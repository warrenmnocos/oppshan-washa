...
The `oppshan-washa` Lambda + execution role + log group, the `AWS_IAM` Function URL + the two CloudFront invoke permissions (`InvokeFunctionUrl` + `InvokeFunction` — OAC→Lambda needs both), the `oppshan-washa-oac` Origin Access Control, the CloudFront distribution (three behaviors + the forwarded-host headers), the **us-east-1** ACM certificate + its DNS validation, the `washa.oppshan.com` Route 53 aliases, the ten `/oppshan/washa/*` SSM parameter slots, and the `oppshan-washa-github-deploy` role.

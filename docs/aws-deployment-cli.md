...
```
Creates, in dependency order: the `oppshan-washa-lambda-exec` role + `/aws/lambda/oppshan-washa` log group → the `oppshan-washa` Lambda (placeholder binary, 256 MB, arm64, reserved concurrency 5) → the `AWS_IAM` Function URL → the `oppshan-washa-oac` Origin Access Control → the **us-east-1** ACM certificate (**ECDSA P-384**; it writes the validation record into the `oppshan.com` zone and waits for issuance) → the CloudFront distribution (three behaviors, the `X-Forwarded-Host`/`-Proto` origin headers) → the `lambda:InvokeFunctionUrl` **and** `lambda:InvokeFunction` permissions scoped to the distribution (OAC→Lambda needs both, or CloudFront gets a 403) → the `washa.oppshan.com` A/AAAA aliases → the `oppshan-washa-github-deploy` role + least-privilege policy → the ten `/oppshan/washa/*` SSM slots (value `REPLACE_ME`).

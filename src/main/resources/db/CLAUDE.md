...
- **Never edit a migration that has run against a *persistent/deployed* database.** Add `V{n+1}__description.sql` instead.
- **Pre-deployment, edit migrations in place — don't pile on versions.** While no persistent/deployed DB has been populated (the current state: only ephemeral Dev Services / Testcontainers DBs that drop-and-recreate each run), `V1`/`V2`/… are still malleable: fold related changes into the existing file and rename if scope shifts, rather than adding a tiny new version. Ephemeral test runs do NOT lock a migration. Once Flyway records a version on a real environment (Neon), treat that file as immutable and start `V{n+1}`.

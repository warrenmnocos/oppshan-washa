...
- **Layout: database `oppshan` (org-level container) / schema `washa` (this app).** Future oppshan apps each get their own schema in the one database. Migration DDL is unqualified — Flyway (`schemas=washa`) + Hibernate (`default-schema=washa`) target the schema; don't hardcode it in SQL.

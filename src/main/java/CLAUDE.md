...
- **Closed string sets are enums, in their own file.** Model a fixed set of string values (types, modes, bases, …) as a Java `enum` in its own file — never a bare `String` field switched on string literals. Constants are **UPPER_CASE** (Java convention); the lowercase wire token is a `value()` exposed via `@JsonValue` + `@JsonCreator` for JSON exchange, while `@Enumerated(STRING)` persists the UPPER_CASE `name()` in the relational column. The TS enum mirrors it (PascalCase constants, same lowercase values). Recreate the column's `CHECK` against the UPPER_CASE set when migrating (e.g. `GoalTargetType` → column stores `OPEN`/`AMOUNT`/`RELATIVE`, JSON carries `open`/…).
...
- **It also persists *across* runs when `testcontainers.reuse.enable=true`** (set on the maintainer's machine — the Dev Services Postgres container is reused, not recreated). So a key that is merely unique *within* a run still accumulates duplicates across repeated local `./mvnw test` runs and eventually fails a unique constraint / `findX` single-result query. Seed-heavy tests must use a **unique-per-run key** (`"sub-" + UUID.randomUUID()`, a random `YearMonth`) or an **idempotent seed** (`findById(...).isEmpty()` guard). Compute-only tests (no persistence) are immune. CI uses a fresh container so this only bites locally.
